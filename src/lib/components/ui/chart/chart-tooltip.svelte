<script lang="ts">
	...
	const visibleSeries = $derived(
		(chartCtx.tooltip.payload as TooltipPayload[]).filter((s) => s.value !== undefined)
	);
	...
	>
		...
		<div class="grid gap-1.5">
			{#each visibleSeries as item, i (item.key + i)}
				...
				{@const indicatorColor = color || (item.config as { color?: string } | undefined)?.color || item.color}
				<div
					class={cn(
