...
>
	<SwitchPrimitive.Thumb
		...
		class={cn(
			"bg-background dark:data-[state=unchecked]:bg-foreground dark:data-[state=checked]:bg-primary-foreground rounded-full group-data-[size=default]/switch:size-4 group-data-[size=sm]/switch:size-3 pointer-events-none block ring-0 transition-transform",
			!checked && "translate-x-0"
		)}
		style={checked ? `transform: translateX(${size === "sm" ? "10px" : "14px"});` : undefined}
	/>
</SwitchPrimitive.Root>
